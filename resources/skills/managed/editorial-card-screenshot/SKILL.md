...
This skill intentionally uses Browserless direct rather than `agent-browser`'s interactive CLI workflow, because the capture step is a one-shot backend screenshot job. If the task turns into iterative page inspection, DOM debugging, or repeated interaction, switch back to `agent-browser -p browserless`.

Do not fall back to local Chrome, local Chromium, or host browser sessions. If Browserless is unavailable, stop and report the screenshot step as blocked.

Before running the script:
- Save the generated HTML to a local file.
- Ensure the page is self-contained except for fonts.
- If the card includes images or icons, inline them or use remote URLs. Browserless receives the HTML as inline content, so local relative assets from the sandbox filesystem will not resolve during capture.
...
- Ensure fallback fonts are declared so layout remains stable even if remote font downloads fail during headless rendering.
...
It uses Browserless' remote screenshot API and requires `BROWSERLESS_API_URL`, `BROWSERLESS_API_KEY`, `curl`, and `python3`.
...
The template keeps a fixed screenshot canvas by default while falling back to readable normal flow on narrower browser widths.
