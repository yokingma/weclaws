...
**做法**:
- `references/swiss-layout-lock.md` 登记的原始版式与当前附带模板是 Swiss 主题的 golden source,但要以**实际页面用法**为准,不要只看未使用的 CSS helper
...
**做法**：
- 大标题、重点 quote、数字大字 → **衬线字体**（中文衬线系统栈 + Playfair Display）
- 正文、描述、pipeline 步骤名 → **非衬线字体**（中文无衬线系统栈 + Inter）
- 元数据、代码、标签 → **等宽字体**（IBM Plex Mono + JetBrains Mono）

模板已内嵌关键拉丁字形；中文继续走系统字体栈，不依赖 Google Fonts。
