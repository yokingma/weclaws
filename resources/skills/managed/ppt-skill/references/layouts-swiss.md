...
本主题的 golden source 是 `references/swiss-layout-lock.md` 登记的原始版式与当前附带模板。生成正文页时不要把 Swiss 当成“自由组合的风格包”。默认只能使用 `S01-S22`。每个 slide 都必须在 `<section>` 上写 `data-layout="Sxx"`。
...
1. 同时打开三份页面:对应参考页、当前 `template-swiss.html` 或生成页、正在修改的测试 PPT。
