...
- 默认路径还会通过 `AGENT_BROWSER_NPM_VERSION` 固定安装 `agent-browser`，镜像同时预装系统 `chromium` 并导出 `AGENT_BROWSER_EXECUTABLE_PATH=/usr/bin/chromium`；非 ARM64 构建继续执行 `agent-browser install --with-deps`，Linux ARM64 则直接回退到系统 Chromium。
