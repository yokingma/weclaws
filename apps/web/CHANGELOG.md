...
- 登录后的用户控制台和管理台已重构为中性企业面板视觉：全局 shell、顶部工具条、创建 bot 表单、列表行、设置页和管理台模块统一使用更低装饰、更高密度的后台布局。
- bot 列表页改为更紧凑的 inventory 行展示，保留搜索、运行状态筛选和 inline rename 行为；第一版不新增独立 Overview 路由、全局搜索、分页或未落地导航。
- bot 详情页新增静态“会话预览”和“工作区文件预览”预留面板；这些面板不读取工作区文件、不新增会话 API，也不改变 SSE 或 runtime intent 语义。
- 共享品牌、页面标题和 bot 名称排版移除负字距，以匹配企业后台的稳定文字排版规范。
